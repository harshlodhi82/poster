...
const getImages = async (option) => {
  ...
  try {
    ...
    // fs.writeFile('imagelink.json', JSON.stringify(images), (err) => {
    //     if (err) throw err;
    //     console.log(success('Saved'));
    // });

    console.log('Browser Closed')
    const links = await scrapImage(images, option.proxy)
    fs.writeFile((Date.now() + 'imagelink.json'), JSON.stringify(links), (err) => {
      if (err) throw err
      console.log('Saved')
    })
    return links
