...
function setup (server) {
  ...
  //   if (server.dropRate > Math.random()) {
  //     socket.destroy()
  //     debug(`randomly dropping ${socket.remoteAddress} on connection`)
  //   }

  //   socket.on('data', data => {
  //     if (server.dropRate > Math.random()) {
  //       socket.destroy()
  //       debug(`randomly dropping ${socket.remoteAddress} on data`)
  //     }
  //   })
  // })
  return server
...
async function onrequest (req, res) {
  this.emit('proxyrequest', req.url)
  debug.request('%s %s HTTP/%s ', req.method, req.url, req.httpVersion)
  var server = this
  var socket = req.socket

  if (server.dropRate > Math.random()) {
    debug(`randomly dropping ${req.url} on request`)
    if (Math.random() < 0.5) {
      // randomly destroy the socket sometimes and sometimes just let time out
      socket.destroy()
    }
    return
  }
  if (server.throttleRate > Math.random()) {
    const ms = getRandomNumberBetween(...server.throttleRange)
    debug(`randomly delaying ${ms}ms ${req.url} on request`)
    await sleep(...server.throttleRange)
  }
  ...
  authenticate(server, req, function (err, auth) {
    ...
    eachHeader(req, function (key, value) {
      ...
      if (!hasXForwardedFor && keyLower === 'x-forwarded-for') {
...
function authenticate (server, req, fn) {
  var hasAuthenticate = typeof server.authenticate === 'function'
  if (hasAuthenticate) {
