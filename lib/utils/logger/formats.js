...
const {
  colorize,
  getColor,
  jsonStringifyErrorReplacer,
  getDateIsoString
} = require('./utils')

const fileFormat = winston.format.printf(info => {
  let message = JSON.parse(JSON.stringify(info.message, jsonStringifyErrorReplacer))
...
const consoleFormat = winston.format.printf(info => {
  const message = JSON.parse(JSON.stringify(info.message, jsonStringifyErrorReplacer))
...
module.exports = {file: fileFormat, console: consoleFormat, json: jsonFormat}
