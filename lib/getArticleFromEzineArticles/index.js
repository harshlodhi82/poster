...
const getArticleFromEzineArticles = async (option) => {
  ...
  try {
    ...
    await page.goto(`https://www.google.com/search?q=site%3Aezinearticles.com+${encodedKeyword}&num=20`)
    ...
    const articles_links = _.reject(articles_data, (v) => !v.id || _.includes(Array.from(option.excludeArticleIds), v.id))

    const articles = []
    for (let i = 0; i < articles_links.length; i++) {
      const res = await getArticle(articles_links[i].url, option.proxy)
...
module.exports = getArticleFromEzineArticles
